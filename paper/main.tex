...
--- including MNIST digit classification (93\% accuracy), image
inpainting (93.8\% accuracy at 30\% observation), and synthetic structure
recovery (ARI~$= 1.0$) --- and present new results on
macroeconomic data (217 countries $\times$ 95 indicators, imputation
$r = 0.987$). The library
...
label (1 column), for a total of 257 columns. We run 6 chains of 800
Gibbs sweeps on 2$\times$ T4 GPUs (3 chains per device, total inference
time 456~seconds).
...
most probable digit label via \texttt{batch\_classify\_column}, \jaxcc{}
achieves 93.0\% classification accuracy on a held-out test set.
SVM baselines achieve 99.9\% (linear) and 96.9\% (RBF) on the same
split. ROC curves per digit are shown in Figure~\ref{fig:mnist_roc}.
...
\caption{(Left) Z-matrix for 257 MNIST features. Two blocks emerge: foreground
  ...
  (Right) Spatial visualization of pixel--label dependence probability.}
...
\caption{Per-digit ROC curves for MNIST classification. \jaxcc{} (6 chains,
  800 sweeps) compared against SVM with linear and RBF kernels.}
\label{fig:mnist_roc}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.6\textwidth]{figures/mnist_inpainting.png}
\caption{Pixel inpainting at 30\% observation. For each digit: original image,
  observed pixels (cyan = missing), posterior predictive $P(\text{pixel}=1)$,
  and thresholded prediction. Accuracy: 93.8\%.}
...
while smaller views isolate inflation/water (2 indicators), migration/transport
(4), environment/economy (7), and singleton outliers.

\paragraph{Dependence matrix.} The Z-matrix (Figure~\ref{fig:wdi_zmatrix})
reveals known macroeconomic relationships: GDP per capita shows high
mutual information with Internet access (MI$=0.41$, Linfoot$=0.75$),
life expectancy (MI$=0.34$), and fertility rate (MI$=0.38$). Life expectancy
and under-5 mortality are strongly coupled (MI$=0.42$).

\paragraph{Imputation.} Holdout evaluation (10\% of observed cells masked)
yields imputation correlation $r = 0.987$ with NRMSE~$= 0.81$.

\paragraph{Anomaly detection.} Venezuela (typicality~$= 0.41$) is identified
as the most atypical country, driven by extreme GDP contraction ($-27.7\%$)
and hyperinflation (real interest rate $-89.5\%$). Most typical: Sri Lanka
(0.97), Greece (0.96), Latvia (0.94).

\begin{figure}[h]
\centering
\includegraphics[width=0.6\textwidth]{figures/wdi_z_matrix.png}
\caption{Z-matrix for 95 World Development Indicators (2019). Hierarchical
  clustering reveals blocks of demographic/health, economic, and infrastructure
  variables with clear independence between blocks.}
