...
$r = 0.987$). The library comprises ${\sim}$10,800 lines of Python, is
validated by 241 tests including 33 Hypothesis property tests, and is
...
split. Per-digit ROC curves are shown in Figure~\ref{fig:mnist_roc}.
...
4~chains of 800~Gibbs sweeps (${\sim}$1.09~s/sweep on P100).
...
\paragraph{Anomaly detection.} Venezuela (typicality~$= 0.41$) is
identified as the most atypical country, driven by extreme GDP
contraction ($-27.7\%$) and hyperinflation (real interest rate
$-89.5\%$). Most typical: Sri~Lanka~(0.97), Greece~(0.96),
Latvia~(0.94).
...
\begin{table}[!htb]
\centering
\caption{Per-sweep wall-clock time on T4 GPU.}
...
$1{,}181\times$ improvement. The full sweep (including the column
assignment kernel not benchmarked individually) achieves $41.8\times$
speedup.
...
17~modules (Table~\ref{tab:modules}). The packed sub-package (5~modules)
