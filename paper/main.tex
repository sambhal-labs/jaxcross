...
achieving substantial speedups through five key systems innovations:
...
The first JIT compilation of \texttt{packed\_gibbs\_sweep} can take
30--120 seconds depending on problem size. We mitigate this via
persistent XLA compilation caching, automatically enabled on import.
...
into 2 views with 3 well-separated Gaussian clusters each. We run 4
chains of 30 Gibbs sweeps.

\paragraph{Results.} Table~\ref{tab:synthetic} shows recovery metrics
averaged over chains. \jaxcc{} reliably recovers the ground truth
structure, with column partition ARI~$\geq 0.80$, row clustering
ARI~$\geq 0.70$ per view, within-view dependence probability~$\geq 0.80$,
and between-view dependence probability~$\leq 0.20$.

\begin{table}[h]
\centering
\caption{Synthetic structure recovery metrics (4 chains, 30 sweeps).}
\label{tab:synthetic}
\begin{tabular}{lcc}
\toprule
Metric & Threshold & Result \\
\midrule
Column partition ARI (mean) & $\geq 0.80$ & \textit{[from benchmark]} \\
Row clustering ARI, view 0 (mean) & $\geq 0.70$ & \textit{[from benchmark]} \\
Row clustering ARI, view 1 (mean) & $\geq 0.70$ & \textit{[from benchmark]} \\
Within-view dependence prob. & $\geq 0.80$ & \textit{[from benchmark]} \\
Between-view dependence prob. & $\leq 0.20$ & \textit{[from benchmark]} \\
\bottomrule
\end{tabular}
\end{table}

% Figures: convergence.png, z_matrix.png, cluster_recovery.png
\begin{figure}[h]
\centering
% \includegraphics[width=0.95\textwidth]{figures/synthetic_convergence.png}
\caption{Convergence of column partition ARI and row clustering ARI over
  30 Gibbs sweeps, averaged across 4 chains. Both metrics stabilize
  within 15 sweeps. \textit{[Figure to be generated from benchmark output.]}}
...
\caption{Inferred dependence probability matrix (Z-matrix) for the
  8-column synthetic dataset. The two $4 \times 4$ diagonal blocks
  correspond to the two ground truth views, with high within-view and
  low between-view dependence probabilities.
  \textit{[Figure to be generated from benchmark output.]}}
...
\caption{Scalability of \jaxcc{} packed Gibbs sweep. (a)~Per-sweep time
  vs.\ number of rows (fixed 10 columns). (b)~Per-sweep time vs.\ number
  of columns (fixed 200 rows). (c)~JIT compilation time vs.\ table size.
  Both axes use log scale. Runtime scales approximately linearly in both
  rows and columns after JIT compilation.
  \textit{[Figure to be generated from scalability benchmark.]}}
\label{fig:scalability}
\end{figure}

\paragraph{JIT compilation cost.} First compilation takes 30--120~seconds
depending on problem size, but is amortized via persistent XLA caching.
Subsequent launches load cached kernels in $<1$~second.
...
a direct wall-clock comparison. However, based on published benchmarks
\citep{mansinghka2016crosscat} and the known sequential Python+C++
architecture, we estimate the packed path provides 10--100$\times$
speedup on GPU hardware for medium-to-large datasets. The kernel-level
benchmark (Table~\ref{tab:kernel_speedup}) compares the unpacked
(pure-Python) path to the packed path within \jaxcc{} itself, showing
the gains from vectorization and GPU compilation.

\begin{table}[h]
\centering
\caption{Kernel-level speedups: unpacked (Python loops) vs.\ packed
  (JIT+GPU) on a $200 \times 10$ dataset with 5 column types.}
\label{tab:kernel_speedup}
\begin{tabular}{lrrr}
\toprule
Kernel & Unpacked (s) & Packed (s) & Speedup \\
\midrule
Row assignments    & \textit{[benchmark]} & \textit{[benchmark]} & \textit{[benchmark]}$\times$ \\
Column hypers      & \textit{[benchmark]} & \textit{[benchmark]} & \textit{[benchmark]}$\times$ \\
CRP alphas         & \textit{[benchmark]} & \textit{[benchmark]} & \textit{[benchmark]}$\times$ \\
\midrule
Full sweep (3 iter) & \textit{[benchmark]} & \textit{[benchmark]} & \textit{[benchmark]}$\times$ \\
\bottomrule
\end{tabular}
\end{table}
