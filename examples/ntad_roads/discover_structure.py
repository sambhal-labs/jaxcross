#!/usr/bin/env python3
"""Discover joint structure in NTAD Texas road sections via jaxcross.
...
Outputs (results/<discovery-dir>/):
    ...
    typicality.csv           per-participant typicality + segment_ids
    anomaly.csv              per-participant anomaly score + segment_ids
...
DEFAULT_INF_DIR = Path("examples/ntad_roads/results/inference")
RESULTS_ROOT = Path("examples/ntad_roads/results")
...
# Curated road-network pairs to compute mutual information for.
# These are pairs where domain knowledge tells us the answer; high MI
# confirms jaxcross learned the correlation, low MI means our model missed it.
MI_PAIRS = [
    # Geometry / capacity couplings
    ("lanes", "speedlim", "Lanes ↔ speed limit (capacity-design coupling)"),
    ("lanes", "class", "Lanes ↔ functional class (Interstate has more lanes)"),
    ("speedlim", "class", "Speed limit ↔ functional class"),
    # Length couplings
    ("length", "shape_length", "Segment length ↔ geometric length (definitional)"),
    ("length", "class", "Length ↔ class (highway segments are longer)"),
    # Network / system couplings
    ("road_system", "class", "Road system ↔ functional class (Interstate=class 1)"),
    ("road_system", "nhs", "Road system ↔ NHS subtype"),
    ("road_system", "speedlim", "Road system ↔ speed limit"),
    ("nhs", "class", "NHS subtype ↔ functional class"),
    # Outcome / label couplings
    ("is_interstate", "road_system", "is_interstate ↔ road_system (definitional)"),
    ("is_interstate", "speedlim", "Interstate ↔ speed limit (75-80 mph design)"),
    ("is_interstate", "lanes", "Interstate ↔ lane count"),
    ("is_interstate", "nhs", "Interstate ↔ NHS subtype"),
    # Admin / ownership
    ("admin_is_state", "class", "State admin ↔ functional class (State carries higher classes)"),
    ("admin_is_state", "speedlim", "State-administered ↔ speed limit"),
    # Border crossings
    ("border", "road_system", "Border ↔ road system (border crossings on US/Interstate)"),
    # Negative controls — should be near zero (no spatial modeling)
    ("centroid_latitude", "lanes", "Latitude ↔ lanes (negative control)"),
    ("centroid_longitude", "speedlim", "Longitude ↔ speed limit (negative control)"),
]

# Labels we'll evaluate per-view row clustering against (with ARI). These
# columns are part of the joint model; ARI tells us which view's row
# clustering aligns best with each label.
LABEL_COLS = [
    "is_interstate",  # headline binary outcome (~12.5% prevalence on TX)
    "class",  # functional class (1-6)
    "nhs",  # NHS subtype (8 levels)
    "road_system",  # I/U/S/C/FM/Other (6 levels)
    "admin_is_state",  # State vs Municipal admin
]
...
def _make_publication_figures(
    ...
    ax.set_title(f"Diabetes 130 dependency matrix sorted by best-chain views ({n_views} views)")
    plt.tight_layout()
...
def _make_advanced_figures(
    ...
    # Continuous columns suitable for in-sample CI-coverage check. Skip
    # the heavily-missing patient_weight_kg (~80 % NaN) — too few cells.
    imputation_targets = [
        "lanes",
        "speedlim",
        "centroid_latitude",
...
def main() -> int:
    ...
    # Load the EXACT training data the chains saw, not the full preprocessed
    # matrix. Phase 2 used --subsample 15000, so the chains' row_assignments
    # have length 15K; using the full 70K matrix would mismatch shapes in
    # cluster-mean computations. train_used.npy is written by run_inference.py
    # for exactly this purpose; train_indices.npy maps it back to segment_ids.
    train_used_path = inf_dir / "train_used.npy"
    full_segment_ids = np.load(PREP_DIR / "segment_ids.npy")
    if train_used_path.exists() and (inf_dir / "train_indices.npy").exists():
        train_data = np.load(train_used_path)
        train_idx = np.load(inf_dir / "train_indices.npy")
        segment_ids = full_segment_ids[train_idx]
        print(
            f"Subsampled training data: {train_data.shape} (subset of "
            f"{full_segment_ids.shape[0]:,} via train_indices.npy)"
        )
    else:
        train_data = np.load(PREP_DIR / "train_data.npy")
        segment_ids = full_segment_ids
        print(f"Full training data: {train_data.shape}")
    n_rows, n_cols = train_data.shape
    ...
    try:
        ...
        plt.colorbar(im, ax=ax, label="P(same view, BMA)")
        ax.set_title(f"Diabetes 130 jaxcross dependency matrix ({n_chains}-chain BMA)")
        plt.tight_layout()
    ...
    # Chunk over rows: batch_row_typicality on 70K rows x 4 chains otherwise
    # tries to allocate >15 GB on a 4 GB GPU. 1K rows per chunk fits comfortably.
    print(f"\n{'=' * 70}\n4. ROW TYPICALITY\n{'=' * 70}")
    all_ids = jnp.arange(n_rows)
    typ_chunks = []
    typ_chunk_size = 1000
    for start in range(0, n_rows, typ_chunk_size):
        end = min(start + typ_chunk_size, n_rows)
        typ_chunks.append(np.asarray(batch_row_typicality(chains, all_ids[start:end])))
    typicality = np.concatenate(typ_chunks)
    print(f"  Range: [{typicality.min():.4f}, {typicality.max():.4f}]")
    print(f"  Mean: {typicality.mean():.4f}, std: {typicality.std():.4f}")
    pl.DataFrame(
        {"segment_ids": segment_ids, "typicality": typicality.astype(np.float32)}
    ).write_csv(out_dir / "typicality.csv")
    ...
    for i in top_typical_idx[:5]:
        print(f"    BridgeID={str(segment_ids[i])}  typicality={typicality[i]:.4f}")
    ...
    print(f"\n{'=' * 70}\n5. ANOMALY SCORE (best chain)\n{'=' * 70}")
    train_jax = jnp.array(train_data)
    # Chunk over rows: same OOM concern as typicality at 70K-row scale
    anom_chunks = []
    anom_chunk_size = 2000
    for start in range(0, n_rows, anom_chunk_size):
        end = min(start + anom_chunk_size, n_rows)
        anom_chunks.append(
            np.asarray(batch_anomaly_score(best_packed, train_jax, all_ids[start:end]))
        )
    anomaly = np.concatenate(anom_chunks)
    print(f"  Range: [{anomaly.min():.3f}, {anomaly.max():.3f}]  mean={anomaly.mean():.3f}")
    pl.DataFrame({"segment_ids": segment_ids, "anomaly": anomaly.astype(np.float32)}).write_csv(
        out_dir / "anomaly.csv"
    )
    ...
    for i in top_anom_idx[:5]:
        print(
            f"    BridgeID={str(segment_ids[i])}  anomaly={anomaly[i]:.3f}  typicality={typicality[i]:.4f}"
        )
    ...
    print(f"  10x10 similarity matrix (anchor segment_ids: {segment_ids[anchor_idx].tolist()}):")
    for i, ai in enumerate(anchor_idx):
        row = sim[i]
        print(
            f"    BridgeID={str(segment_ids[ai])}  min={row.min():.3f}  "
            f"mean={row.mean():.3f}  max={row.max():.3f}"
        )
    sim_cols: dict = {"anchor_sample_id": segment_ids[anchor_idx]}
    for j, aj in enumerate(anchor_idx):
        sim_cols[f"sim_to_sample_{str(segment_ids[aj])}"] = sim[:, j].astype(np.float32)
    pl.DataFrame(sim_cols).write_csv(out_dir / "similarity_anchors.csv")

    print("\n  Top-5 nearest neighbours per anchor (across full cohort):")
    # Chunked: per-anchor similarity to all rows in 500-row chunks.
    # Avoids the n_rows^2 allocation that OOMs a 4 GB GPU.
    chunk_size = 500
    ...
    for j, ai in enumerate(anchor_idx):
        ...
        nearest_rows.append(
            {
                "anchor_sample_id": str(segment_ids[ai]),
                "nn1_sample_id": str(segment_ids[nn[0]]),
                "nn1_sim": float(row[nn[0]]),
                "nn2_sample_id": str(segment_ids[nn[1]]),
                "nn2_sim": float(row[nn[1]]),
                "nn3_sample_id": str(segment_ids[nn[2]]),
                "nn3_sim": float(row[nn[2]]),
                "nn4_sample_id": str(segment_ids[nn[3]]),
                "nn4_sim": float(row[nn[3]]),
                "nn5_sample_id": str(segment_ids[nn[4]]),
                "nn5_sim": float(row[nn[4]]),
            }
        )
        print(
            f"    Anchor BridgeID={str(segment_ids[ai])}: "
            f"NN={[str(segment_ids[k]) for k in nn]}  sims={[round(float(row[k]), 3) for k in nn]}"
        )
