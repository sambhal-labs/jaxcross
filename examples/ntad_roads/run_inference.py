...
Outputs (examples/ntad_roads/results/inference/<fd>/):
...
Run a 5-minute smoke test first to validate the pipeline end-to-end:
    ...
    uv run python examples/ntad_roads/run_inference.py --smoke
...
PREP_ROOT_DEFAULT = Path("examples/ntad_roads/results/preprocessed")
OUT_ROOT_DEFAULT = Path("examples/ntad_roads/results/inference")
...
def _pack_initial_chains(
    ...
    # data= flips silent jnp.clip into a ValueError if any categorical/ordinal
    # column has values >= max_categories. county_class currently has 11
    # distinct levels so 16 fits with headroom, but adding new TX years or
    # widening top-N county bucketing would otherwise corrupt suffstats silently.
    return [
        pack_state(
            s,
            max_views=max_views,
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=int,
        default=16,
        help="Max categorical/ordinal levels per column. Current preprocessing "
        "produces at most 11 (county_class), so 16 has headroom. Bump if you "
        "expand top-N county bucketing or add new NTAD years.",
    )
    ...
    parser.add_argument(
        ...
        "'examples/ntad_roads/results/preprocessed'. Set this to "
        "'examples/ntad_roads/results/preprocessed_holdout' for the "
        "held-out evaluation run.",
    )
    args = parser.parse_args()

    # Production batch script — surface any max_cols_per_view runtime overflow
    # as a hard error rather than a silent column drop with a debug callback.
    ...
    print(
        f"Config: ntad_roads, {n_chains} chains x {args.sweeps} sweeps, "
