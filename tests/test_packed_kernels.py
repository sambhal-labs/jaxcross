...
@pytest.mark.xfail(
    reason=(
        "pack_state() now validates max_cols_per_view at pack time (packed/state.py:312-317), "
        "so the overflow state this test tries to construct can no longer be built via "
        "pack_state. The runtime warning path in packed_transition_column_assignments is "
        "still reachable but needs a different setup (direct PackedCrossCatState construction "
        "with matching array shapes). Tracked as follow-up; surfaced by expanded CI coverage."
    ),
    strict=True,
)
